...
client.on('messageCreate', async (message) => {
    ...
    // FORCED ADD - NOW ACCEPTS ID OR NAME
...
**Commands:**
• \`%pattern add <pokemon> <category name or ID>\` - Add manual pattern
• \`%pattern remove <pokemon>\` - Remove pattern
• \`%pattern list\` - List all manual patterns
• \`%patterns\` - Show stats
• \`%forced add <category name or ID>\` - ONLY process this category
• \`%forced list\` - List forced categories
• \`%forced remove <category name or ID>\` - Remove from forced
• \`%ignore add <category name or ID>\` - NEVER process this category
• \`%ignore list\` - List ignored categories
• \`%ignore remove <category name or ID>\` - Remove from ignore
...
**Tip:** You can use category ID instead of name for special characters!`);
    }
});
